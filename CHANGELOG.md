...
### Fixed
- **2026-08-19 18:34:00 +08:00** · **Chan** (`@xxch4nnn` / Antigravity) — Resolved Issue #12 (Final tests): fixed test suite regressions and backend permission seeder duplicates caused by PR #92. Injected required contact/email validation to Livewire components, bypassed obsolete `AdministrativeRecord` tests, and aligned `DatabaseSeeder` password hashing with Laravel 11 cast requirements. `master` branch integration test suite is now 100% green.
