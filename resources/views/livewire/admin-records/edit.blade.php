<x-slot name="header">
    <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Administrative Record:') }} {{ $record->title }}
        </h2>
        <a href="{{ route('admin-records.show', $record->id) }}" class="text-indigo-600 hover:text-indigo-900 text-sm font-medium">
            &larr; Back to Record
        </a>
    </div>
</x-slot>
        ...
        <div class="bg-white/70 backdrop-blur-xl border border-white/50 overflow-hidden shadow-xl sm:rounded-2xl p-6">
            <form wire:submit="save" class="space-y-6">
                ...
                <div class="mt-8 flex justify-end gap-3 pt-6 border-t border-gray-200/50">
                    <a href="{{ route('admin-records.show', $record->id) }}" class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Cancel</a>
